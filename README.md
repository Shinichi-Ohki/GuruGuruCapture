...
| 内側をドラッグ | 選択範囲を移動 |
| `Enter` or ダブルクリック | **確定してキャプチャ！**（カーソルなし） |
| `Shift` + 確定 | **カーソル付きでキャプチャ！** |
| `Command` + 確定 | **OCR / QRデコード！** テキストを認識、またはQRコード・バーコードをデコードしてクリップボードにコピー |
| `Esc` or 右クリック | キャンセル |
...
### OCR・QRモード
範囲選択後に `Command` + 確定（ダブルクリック or Enter）で認識モード。
**QRコード・バーコード**（EAN、Code128、Aztec、PDF417など）が含まれていればデコードした文字列を、なければ**テキストOCR**の結果をクリップボードにコピーします。
画像の保存は行いません。
